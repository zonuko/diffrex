...
「**Deno ネイティブで動作し、AI 時代のコードレビュー体験を支援する差分ツール**」を目指して名付けられました。
...
- **TypeSafe Jev (System One) セマンティック解析**: TypeSafe AI の Jev モデルを用い、プロンプト指示との合致度、潜在的ロジック破壊、装飾的ノイズを超高速・非ブロッキングに判定。未設定時・オフライン時は静的解析へ自動フォールバック。
- **Confidence-Gated ハイブリッドレビュー & オンデマンド深掘り解説**: 確信度スコアに応じた自動トリアージ（確信度85%以上の安全差分は Safe バッジ表示で一括承認候補、低確信度・高リスク差分は「要精査」警告）。疑義 Hunk 上の「🔍 Explain」ボタンから詳細な自然言語解説をオンデマンド生成。メニューから確信度しきい値も自由に調整可能。
...
| `--no-restore` | 起動時の自動セッション復元を無効化 |
| `--welcome` | 強制的に Welcome / 選択画面で起動 |
| `-h`, `--help` | ヘルプメッセージを表示 |
...
| 環境変数 | 説明 |
| :--- | :--- |
| `TYPESAFE_API_KEY` | TypeSafe AI Jev System One によるセマンティック差分解析を有効化する API キー |
| `TYPESAFE_API_URL` | Jev API のカスタムエンドポイント URL（通常は省略可能） |
| `OPENAI_API_KEY` | （任意）疑義 Hunk のオンデマンド深掘り解説（System Two LLM）に直接接続する API キー |
| `ANTHROPIC_API_KEY` | （任意）疑義 Hunk のオンデマンド深掘り解説（System Two LLM）に直接接続する API キー |
